...
import { successResponse, contentReportSerializer, contentGuidelinesSerializer, contentModerationSerializer, safetyTipsSerializer } from "#src/serializers/index.js";
import { formatError, aiService } from "#src/helpers/index.js";

const router = express.Router({ mergeParams: true });

const reportContentValidators = [
  body("content_type").isIn(["artifact", "input", "shared_view"]).withMessage("Invalid content type"),
  body("content_id").isUUID().withMessage("Valid content ID is required"),
  body("reason").isIn(["inappropriate", "offensive", "spam", "copyright", "other"]).withMessage("Invalid report reason"),
...
router.post(
  ...
  async (req, res) => {
    try {
      const { content_type, content_id, reason, description, metadata = {} } = req.body;
      ...
      if (!content) {
        return res.status(404).json(formatError("Content not found or not accessible", 404));
      }
      ...
      return res.status(200).json(successResponse(data, "Content report submitted successfully"));
    } catch (error) {
      console.error("Report content error:", error);
      return res.status(500).json(formatError("Failed to submit content report"));
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      const guidelines = app.config?.content_safety?.guidelines || {
        overview: "We're committed to creating a safe, positive environment for children and families.",
        ...
        reporting: {
          how_to_report: "Use the report button on any content that violates these guidelines",
          what_happens: "Reports are reviewed within 24 hours by our safety team",
          follow_up: "We'll take appropriate action and notify you of the outcome",
        },
      };

      const data = contentGuidelinesSerializer(app, guidelines);

      return res.status(200).json(successResponse(data, "Content safety guidelines retrieved successfully"));
    } catch (error) {
      console.error("Get content guidelines error:", error);
      return res.status(500).json(formatError("Failed to retrieve content guidelines"));
    }
  }
);
...
router.post(
  ...
  body("text").isLength({ min: 1, max: 2000 }).withMessage("Text must be 1-2000 characters"),
  handleValidationErrors,
  async (req, res) => {
    try {
      const { text } = req.body;
      ...
      return res.status(200).json(successResponse(data, "Content safety check completed"));
    } catch (error) {
      console.error("Content safety check error:", error);
      return res.status(500).json(formatError("Failed to check content safety"));
    }
  }
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      return res.status(200).json(successResponse(data, "Safety tips retrieved successfully"));
    } catch (error) {
